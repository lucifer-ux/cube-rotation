...
cubes =
  ...
  where
    ...
    eval = case _ of
      ...
      ForwardToggle next -> do
        cube <- H.get
        let a= cube.forward
        H.modify
          (\c ->
            -- c { forward = not cube.forward }  
            c { forward = (if a then false else true) }  
          )
        pure next
      
      VelocityIncrement next -> do
        cube <- H.get
        let a= cube.speed
        H.modify
          ( \c ->
...
-------------------- VIEW --------------------
renderView ::forall m.RotatingState -> H.ComponentHTML Action () m
renderView state = let
  ...
  in
    HH.div [] $
      ...
      , renderButton "change-dir" (ForwardToggle)
      , renderButton "Vel++" (VelocityIncrement)
      , renderButton "Vel--" (VelocityDecrement)
      , HH.text ( show state.speed )
      , renderButton "AddCuber" (AddCube)
